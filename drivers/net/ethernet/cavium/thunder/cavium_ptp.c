			    ...
			    const struct pci_device_id *ent)
{
	...
	if (IS_ERR_OR_NULL(rst)) {
		dev_info(dev, "Can not find RST.  Use default clock rate");
		rst = NULL;
		clock_rate = cavium_rst_clock_rate_default;
	} else {
		clock_rate = cavium_rst_get_clock_rate(rst);
	}
