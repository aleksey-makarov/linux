...
FORMAT_PATCH_TAG=afpf
# FORMAT_VERSION=2 # [PATCH vXXX]

submit_format() {

	OG_LN=outgoing.${FORMAT_PATCH_TAG}
	OG=${OG_LN}.${DATE}
...
# git send-email --no-chain-reply-to --cc-cmd='./scripts/get_maintainer.pl --interactive --no-git-fallback' \
git send-email --no-chain-reply-to --suppress-cc=all \
	--to="Linu Cherian <linu.cherian@cavium.com>" \
	--cc='"Goutham, Sunil" <Sunil.Goutham@cavium.com>' \
	--cc="Aleksey Makarov <aleksey.makarov@cavium.com>" \
...
#	--dry-run \
#	--cc='"Akula, Geethasowjanya" <Geethasowjanya.Akula@cavium.com>' \
